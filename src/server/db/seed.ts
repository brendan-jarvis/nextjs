...
function readMdxFiles(dir: string): Array<{ frontmatter: Frontmatter; body: string; filename: string }> {
...
async function seed() {
  ...
  for (const { frontmatter, body, filename } of blogFiles) {
    ...
    const [inserted] = await db
      .insert(posts)
      .values({
        authorId: (frontmatter.authors?.[0] || "brendanjarvis").toLowerCase().replace(/\s+/g, ""),
  ...
  // Seed Projects
  for (const { frontmatter, body, filename } of projectFiles) {
    const date = new Date(frontmatter.date);
    const [inserted] = await db
      .insert(projects)
      .values({
  ...
  if (motorcyclePostId) {
    ...
    console.log(`Inserted test comment from Grok on post ${motorcyclePostId} (comment id: ${comment.id})`);
  }
